...
and hands over an **Action Plan** — _"send this to these 15 High Intent
contacts"_, _"post this on LinkedIn"_ — which a company would execute in its own
...
depending on event _order_ or _timing_ — attribution paths, journey length — is
...
modules/
  ...
  m4_content/       content pipeline  crawl → knowledge base → goal/tone →
                    generate → score → optimize, plus its Streamlit dashboard
...
> _and_ PostgreSQL; all five figures show PostgreSQL, and that is what is used.
...
whether the action _changes_ what they do. On — the Hillstrom dataset 64,000
...
| predicted response _(what the recommender did)_ | 75.9 | [38.8, 110.0] |                              91 |
| random targeting                                | −9.8 | [−48.7, 27.0] |                              58 |

**Stated carefully:** the intervals overlap, and the second uplift learner does
_worse_ than — the current policy so "use uplift modelling" is not a conclusion
...
can answer the right one for _your_ actions. So the system now records what it
...
deterministic log looks _more_ trustworthy, not less. Its effective sample size
...
| 5   | **Goal/tone corpus needlessly quartered**                     | A row had to clear the confidence bar for _both_ targets although the classifiers train separately. **114 → 527 rows.**                                                                             |
...
- **No action was ever randomised on this audience**, so the next-best-action
  ...
  _was_ randomised; those actions are not this project's actions.
...
Here's the command for each part, in the order you'd start them. Run each in its own terminal from the repo root:

# What Command

1 PostgreSQL (must be first) make db
