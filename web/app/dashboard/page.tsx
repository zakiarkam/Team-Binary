...
export default async function OverviewPage() {
  ...
  // Drafts from the Action Plan have sent nothing — they cannot be "best".
  const campaigns = (campaignList?.campaigns ?? []).filter(
    (c) => c.status !== "draft",
  );
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Kpi
          label="CONTACTABLE"
          value={reach?.reachable.toLocaleString() ?? "0"}
        ...
        <Kpi
          ...
          hint="from tracked campaigns"
          tone="accent"
        />
        <Kpi
          label="CONTENT ASSETS"
          value={content?.count ?? 0}
          hint="scored and stored"
          tone="warn"
        />
      </section>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <FunnelChart
            funnel={
              funnel?.funnel ?? { sent: 0, open: 0, click: 0, convert: 0 }
            }
          />
        </Card>
      </div>
      ...
      {best && (
        <Card className="mt-4">
          ...
          <Caveat>
            ...
            Operational complexity is shown beside it, because a policy that
            wins narrowly while needing eight rules instead of one is not
            obviously better.
          </Caveat>
        </Card>
      )}
