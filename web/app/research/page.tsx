  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Kpi label="DATASETS" value={summary.n_datasets}
             hint={`${summary.n_present} present on disk`} />
        <Kpi label="REAL-WORLD DATA" value={summary.real_datasets}
             hint="observed from real people" tone="ok" />
        <Kpi label="SIMULATED / SYNTHETIC" value={summary.simulated_or_synthetic}
             hint="generated, not observed" tone="warn" />
        <Kpi label="UNUSED" value={summary.n_unused}
             hint="shipped but read by no code" tone="bad" />
      </section>
      ...
      <Card className="mt-4">
          ...
          columns={[
            {
              ...
              render: (d) => (
                <div>
                  ...
                  <p className="font-mono text-[0.7rem] text-slate-400">{d.path}</p>
                </div>
              ),
            },
            {
        ...
        <Caveat>
          ...
          engagement model is the obvious use for them, and until that exists the
          engagement regressor rests on a synthetic corpus.
        </Caveat>
      </Card>
          ...
          {prov.models.map((m) => (
            <div key={m.key} className="rounded-xl border border-slate-200 p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {!m.artifact_present && <Pill tone="bad">artifact missing</Pill>}
              </div>
      ...
      {prov.goal_tone_metrics && (
        <Card className="mt-4">
          ...
          <Caveat>
            ...
            does well on the common classes and poorly on the rare ones, which an
            accuracy figure alone would hide.
          </Caveat>
        </Card>
      )}
      ...
      <Card className="mt-4">
        ...
        <ul className="mt-3 space-y-2 text-sm leading-relaxed text-slate-600">
          ...
          <li>
            <b>The conversion and drop-off models were fitted on a simulator.</b>{" "}
          ...
          <li>
            <b>The goal and tone classifiers are low-data.</b> ~113 labelled rows
            is small enough that a single train/test split is noisy.
          </li>
          <li>
